...
#metrics 
#can run for missing sims (when hyak runs out of memory)
#or one sim per loop
...
#
sim_id_ref_range<-seq(from = 1, to = n_sims, by = 20)
...
outdir<-paste0('/gscratch/icrc/cgreene3/calibration_outputs/')
# 
...
# 
# #location where want outputs
# outdir<-paste0(here(), '/test/calibration_outputs')
...
#if running because hyak ran out of memory so need to run sim one by one
missing_sims_df<-read.csv('missing_sims_df.csv')
missing_sims<-missing_sims_df$sim_id
sim_id_current_eval<-missing_sims[args_temp]

# #if running for each sim
# sim_id_current_eval<-args_temp
...
HIV_transitions_param_func<-function(yr, N_t_r_h_g){
  ...
  for (g in G_SET){
    ...
    if(yr >= 2004){
      ...
      #ART coverage (not calibrated and time varying) #sigma_g(\tau)
...
#write.csv(out_df%>%filter(year >= 1990), file = paste0('out_df_sim_id_', sim_id_current_eval, '.csv'), row.names = FALSE)
...
summarised_metrics_TB<-out_df%>%
  ...
  summarise(Tb_inc_neg_female_Y_per_100k_female = sum(Tb_inc_neg_female_t_per_100k_female),
            ...
            Tb_inc_pos_female_Y_per_100k_femal = sum(Tb_inc_pos_female_t_per_100k_female),
...
setwd(outdir)
write.csv(calib_metrics_df_all_df, file = paste0('calib_metrics_df_sim_id_', 
                                                 sim_id_current_eval,
                                                 '.csv'), row.names = FALSE)
