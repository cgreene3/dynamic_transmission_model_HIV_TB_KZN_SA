...
sim_id_start<-sim_id_ref_range[args_temp]
sim_id_end<-sim_id_start+19
...
for(sim_id_current_eval in sim_id_start:sim_id_end){  
  ...
  #out_df_melt<-out_df%>%
  #  filter(year >= 1990)
  #out_df_melt<-melt(out_df_melt, id = c("time", "year", "sim_id"))
  #write.csv(out_df_melt, file = paste0('out_df_sim_id_', sim_id_current_eval, '.csv'), row.names = FALSE)
  ...
  if(sim_id_current_eval_temp == sim_id_start){
...
setwd(outdir)
write.csv(calib_metrics_df_all_df, file = paste0('calib_metrics_df_from_', sim_id_start,
                                                 '_to_', sim_id_end, '.csv'), row.names = FALSE)
