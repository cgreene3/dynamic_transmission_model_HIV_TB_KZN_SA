...
indir_outputs<-paste0('/gscratch/icrc/cgreene3/calibration_outputs/run_', run_id)
...
outdir<- paste0('/gscratch/icrc/cgreene3/calibration_analysis/run_', run_id)
...
best_results_df_beta<-results_df_summarised_selection_options%>%
  filter(total_in_confidence >= 18)%>%
  mutate(run_id = run_id)

best_results_df_accepted<-results_df_summarised_selection_options%>%
  filter(total_in_confidence == 20)

results_df_accepted_points<-results_df%>%
  filter(sim_id %in% best_results_df_accepted$sim_id)%>%
  mutate(run_id = run_id)

setwd(outdir)
if(run_id > 1){
  results_df_accepted_points_all<-read.csv('results_df_accepted_points_all.csv')
  results_df_accepted_points_all<-rbind(results_df_accepted_points_all, results_df_accepted_points)
} else {
  results_df_accepted_points_all<-results_df_accepted_points
}

write.csv(results_df_accepted_points_all, 'results_df_accepted_points_all.csv')

if(run_id > 1){
  best_results_df_beta_all<-read.csv('best_results_beta_dist.csv')
  best_results_df_beta_all<-rbind(best_results_df_beta_all, best_results_df_beta)
} else {
  best_results_df_beta_all<-best_results_df_beta
}

write.csv(best_results_df_beta_all, 'best_results_beta_dist.csv')

write.csv(missing_sims_df, 'missing_sims_df.csv')

#write.csv(best_results_df, 'best_results_df_summarised.csv')
#write.csv(results_df_summarised_selection_options, 
#          'results_df_summarised.csv')
