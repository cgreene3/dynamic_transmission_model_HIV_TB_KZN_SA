...
library(here)
indir_outputs<-paste0(here(), '/results/program_outputs_for_cost_model_1708/program_outputs_economic_analysis')
...
indir_delay_data<-paste0(here(), '/results/calibration_analysis_1708')
outdir_results<-paste0(here(), '/results/cost_model_updated_IPT_costing')
...
input_costs_ref_df<-input_costs_ref_df%>%
  mutate(variable = if_else(variable == 'n_ipt', 'n_ipt_init_t', as.character(variable)))
...
#for (i in c(1, 100:200)){
  ...
  #n initializing IPT
  model_outputs_temp_df$n_ipt_init_t<-if_else(model_outputs_temp_df$time == 0, 0, 
                                       model_outputs_temp_df$IPT_init_cumulative - lag(model_outputs_temp_df$IPT_init_cumulative))
...
png(file="tornado_plot_cost_sensitivity.png")#,
    #width=600, height=500)
print(tornado_plot_cost_sensitivity)
dev.off()
