...
library(here)
indir_outputs<-paste0(here(), '/results/program_outputs_for_cost_model/program_outputs_economic_analysis')
...
outdir_results<-paste0(here(), '/results/cost_model')
...
cost_itemize_total_df<-data.frame() #costs by line item (IPT, ART, TB care, O HIV care)
cost_itemize_by_year_df<-data.frame() #costs by line item and year
...
mort_yearly_df<-data.frame() #by year
inc_total_df<-data.frame() #total inc
n_ipt_active_DSTB_df<-data.frame() #nipt
recent_remote_df<-data.frame()
hiv_less_more_df<-data.frame()
...
for (i in 1:length(all_files_in_outdir)){
  ...
  if(!(sim_id_temp %in% remaining_calib_sets_df$sim_id)){
    print('filtered')
  }
  ...
  if(sim_id_temp %in% remaining_calib_sets_df$sim_id){
    ...
    #active TB from yearly costs to 1/(upsilon*pi_{6,7}) -- monthly costs
    ...
    durations_ref_df_temp<-rbind(durations_ref_df_temp,
                             data.frame(duration_adj_desc = 'tb_active_duration_adj_h1',
                                        duration_adj = tb_h1_duration_temp/12))
    durations_ref_df_temp<-rbind(durations_ref_df_temp,
                             data.frame(duration_adj_desc = 'tb_active_duration_adj_h2',
                                        duration_adj = tb_h2_duration_temp/12))
    durations_ref_df_temp<-rbind(durations_ref_df_temp,
                             data.frame(duration_adj_desc = 'tb_active_duration_adj_h3',
                                        duration_adj = tb_h3_duration_temp/12))
    durations_ref_df_temp<-rbind(durations_ref_df_temp,
                             data.frame(duration_adj_desc = 'tb_active_duration_adj_h4',
                                        duration_adj = tb_h4_duration_temp/12))
    durations_ref_df_temp<-rbind(durations_ref_df_temp,
                                 data.frame(duration_adj_desc = 'none',
                                            duration_adj = 1))
    ...
    #n ART init
    model_outputs_temp_df$n_art_init_t<-if_else(model_outputs_temp_df$time == 0, 0, 
                                                model_outputs_temp_df$ART_init_cumulative - lag(model_outputs_temp_df$ART_init_cumulative))
    ...
    cost_model_temp_df_join_avg<-cost_model_temp_df_melt%>%
      full_join(input_costs_ref_df%>%mutate(variable = as.character(variable)),
                by = c('variable'))%>%
      mutate(duration_adj_desc = as.character(duration_adj_desc))%>%
      left_join(durations_ref_df_temp%>%mutate(duration_adj_desc = as.character(duration_adj_desc)), 
                by = c('duration_adj_desc'))%>%
      ...
      #filter(variable != 'n_art_init_t')
    
    #######itemize average cost by program, line item and year #######
    cost_itemize_df_temp_by_year<-cost_model_temp_df_join_avg%>%
      ...
      summarise(undisc_cost_itemized = sum(undisc_val_cost_duration_adj),
                ...
                n_individuals_incuring_cost = sum(value))
    
    cost_itemize_df_temp_total<-cost_itemize_df_temp_by_year%>%
      group_by(sim_id, program_id, cost_desc)%>%
      summarise(undisc_cost_itemized_total = sum(undisc_cost_itemized),
                disc_cost_itemized_total = sum(disc_cost_itemized),
                n_individuals_incuring_cost_total = sum(n_individuals_incuring_cost))
    ...
    cost_total_df_temp_avg<-cost_model_temp_df_join_avg%>%
      ...
      mutate(sensitivity_id = 0)
    
    if (i == 1){
      cost_total_df<-cost_total_df_temp_avg
      cost_itemize_by_year_df<-cost_itemize_df_temp_by_year
      #cost_itemize_total_df<-cost_itemize_df_temp
    } else {
      cost_total_df<-rbind(cost_total_df, cost_total_df_temp_avg)
      cost_itemize_by_year_df<-rbind(cost_itemize_by_year_df, cost_itemize_df_temp_by_year)
    }
    ...
    for (s in unique(cost_sensitivity_scenarios_df$sensitivity_id)){
      ...
      cost_model_temp_df_join_sensitivity_temp<-cost_model_temp_df_melt%>%
        ...
        full_join(input_costs_ref_df_sensitivity%>%mutate(variable = as.character(variable)), 
                  by = c('variable'))%>%
        mutate(duration_adj_desc = as.character(duration_adj_desc))%>%
        left_join(durations_ref_df_temp%>%mutate(duration_adj_desc = as.character(duration_adj_desc)), 
                  by = c('duration_adj_desc'))%>%
    ...
    ###pull n_ipt and active TB overtime###
    n_ipt_active_DSTB_temp_df<-model_outputs_temp_df%>%
      select(c('sim_id', 'year', 
               'time', 'program_id', 
               'n_ipt', 'n_tb_r1_h1',
               'n_tb_r1_h2',
    ...
    if (i == 1){
      n_ipt_active_DSTB_df<-n_ipt_active_DSTB_temp_df
    } else {
      n_ipt_active_DSTB_df<-rbind(n_ipt_active_DSTB_df, n_ipt_active_DSTB_temp_df)
    }
    ...
    YLD_temp_df_calc<-YLD_temp_df_melt%>%
      mutate(compartment_id = as.character(compartment_id))%>%
      left_join(disability_weights_ref_df%>%mutate(compartment_id = as.character(compartment_id)),
                by = c('compartment_id'))%>%
      left_join(disc_factor_ref_df, by = c('year'))%>%
    ...
    #get yearly mortality
    mort_yearly_df_temp<-YLL_df_temp_melt%>% #YLL df has mort values.
      filter(!grepl('O_mort', variable))%>%
      mutate(time = time + 2018 - 0.083333)%>% ###to convert time to 2018.083333 = January 2018, etc.
      mutate(year = floor(time)) #match year with converted time for discounting.
    
    mort_total_df_temp<-mort_yearly_df_temp%>% 
    ...
    if (i == 1){
      ...
      mort_yearly_df<-mort_yearly_df_temp
    } else {
      mort_total_df<-rbind(mort_total_df, mort_total_df_temp)
      mort_yearly_df<-rbind(mort_yearly_df, mort_yearly_df_temp)
    }
    ...
    #calculate % recent vs remote 
    model_outputs_temp_df$n_recent<-rowSums(model_outputs_temp_df[,4+c(N_t_r_h_g_ref[3, DR_SET, HIV_SET, G_SET])])
    model_outputs_temp_df$n_remote<-rowSums(model_outputs_temp_df[,4+c(N_t_r_h_g_ref[4, DR_SET, HIV_SET, G_SET])])
    
    recent_remote_df_temp<-model_outputs_temp_df%>%
      filter(time == 0)%>%
      filter(program_id == 1)%>%
      mutate(percent_recent = n_recent/(n_recent+n_remote),
             percent_remote = n_remote/(n_recent+n_remote))%>%
      select(c('sim_id', 'percent_recent', 'percent_remote'))
    
    if(i == 1){
      recent_remote_df<-recent_remote_df_temp
    } else {
      recent_remote_df<-rbind(recent_remote_df, recent_remote_df_temp)
    }
    
    #calculate % less vs more CD4 HIV not on ART 
    model_outputs_temp_df$n_CD4_more<-rowSums(model_outputs_temp_df[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, 2, G_SET])])
    model_outputs_temp_df$n_CD4_less<-rowSums(model_outputs_temp_df[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, 3, G_SET])])
    
    hiv_less_more_df_temp<-model_outputs_temp_df%>%
      filter(time == 0)%>%
      filter(program_id == 1)%>%
      mutate(percent_CD4_more = n_CD4_more/(n_CD4_more+n_CD4_less),
             percent_CD4_less = n_CD4_less/(n_CD4_more+n_CD4_less))%>%
      select(c('sim_id', 'percent_CD4_more', 'percent_CD4_less'))
    
    if(i == 1){
      hiv_less_more_df<-hiv_less_more_df_temp
    } else {
      hiv_less_more_df<-rbind(hiv_less_more_df, hiv_less_more_df_temp)
    }
...
######ICERs SENSITIVITY COST AND TORNADO PLOT#######
...
ICERS_sensitivity_cost_df3<-ICERS_sensitivity_cost_df2%>%
  select(c('cost_desc_graph', 'sensitivity_desc', 'est'))

ICERS_sensitivity_cost_df4<-dcast(ICERS_sensitivity_cost_df3,
                                  cost_desc_graph ~ sensitivity_desc)

ICERS_sensitivity_cost_df4$UI<-abs(ICERS_sensitivity_cost_df4$high-ICERS_sensitivity_cost_df4$low)


ICERS_sensitivity_cost_df4<-ICERS_sensitivity_cost_df4%>%
  mutate(UI_rank = rank(UI))%>%
  arrange(UI_rank)

width <- 0.95
type<-ICERS_sensitivity_cost_df4$cost_desc%>% unlist() %>% levels()


tornado_plot_df<-ICERS_sensitivity_cost_df4%>%
  mutate(xmin=as.numeric(UI_rank)-width/2,
         xmax=as.numeric(UI_rank)+width/2)%>%
  select(-c('UI', 'UI_rank'))

tornado_plot_df_melt<-melt(tornado_plot_df, id = c("cost_desc_graph",
                                                   "xmin", "xmax"))
...
cost_desc_graph = gsub("\\n","\n",tornado_plot_df2$cost_desc_graph,fixed=T)


###TORNADO PLOT ICER 1 vs 3#####
tornado_plot_cost_sensitivity_p3_p1_disc_DALYS<-ggplot() + 
  ...
  geom_hline(yintercept = 590, linetype=2, color = "grey", size = 1.25)+
  scale_x_continuous(breaks = c(1:length(tornado_plot_df2$cost_desc_graph)), 
                     labels = cost_desc_graph) +
  coord_flip()+
  ylab('Discounted incremental cost per DALY averted')
...
#####TORNADO GRAPH COMPARING PROGRAMS 2 and 3######
# p3_p2_sensitivity_cost_df<-incremental_ICER_calcs_df_cast%>%
#   select(c("sim_id", 
#            "sensitivity_id",
#            "p3_disc_cost_total", 
#            "p2_disc_cost_total"))%>%
#   mutate(incremental_disc_cost = p3_disc_cost_total - p2_disc_cost_total)%>%
#   group_by(sensitivity_id)%>%
#   summarise(est = mean(incremental_disc_cost))
# 
# p3_p2_sensitivity_cost_df<-p3_p2_sensitivity_cost_df%>%
#   left_join(cost_sensitivity_scenarios_df, by = c("sensitivity_id"))
# 
# p3_p2_avg_cost<-filter(p3_p2_sensitivity_cost_df, sensitivity_id == 0)
# 
# p3_p2_sensitivity_cost_df2<-filter(p3_p2_sensitivity_cost_df, 
#                                    sensitivity_id != 0)
# 
# p3_p2_sensitivity_cost_df3<-p3_p2_sensitivity_cost_df2%>%
#   select(c('cost_desc_graph', 'sensitivity_desc', 'est'))
# 
# p3_p2_sensitivity_cost_df4<-dcast(p3_p2_sensitivity_cost_df3,
#                                   cost_desc_graph ~ sensitivity_desc)
# 
# p3_p2_sensitivity_cost_df4$UI<-abs(p3_p2_sensitivity_cost_df4$high-p3_p2_sensitivity_cost_df4$low)
# 
# 
# p3_p2_sensitivity_cost_df4<-p3_p2_sensitivity_cost_df4%>%
#   mutate(UI_rank = rank(UI))%>%
#   arrange(UI_rank)
# 
# width <- 0.95
# type<-p3_p2_sensitivity_cost_df4$cost_desc%>% unlist() %>% levels()
# 
# 
# p2_p3_cost_sensitivity_tornado_plot_df<-p3_p2_sensitivity_cost_df4%>%
#   mutate(xmin=as.numeric(UI_rank)-width/2,
#          xmax=as.numeric(UI_rank)+width/2)%>%
#   select(-c('UI', 'UI_rank'))
# 
# p2_p3_cost_sensitivity_tornado_plot_df_melt<-melt(p2_p3_cost_sensitivity_tornado_plot_df, id = c("cost_desc_graph",
#                                                    "xmin", "xmax"))
# 
# p2_p3_cost_sensitivity_tornado_plot_df_melt$avg_cost <-p3_p2_avg_cost$est
# 
# p2_p3_cost_sensitivity_tornado_plot_df_melt2<-p2_p3_cost_sensitivity_tornado_plot_df_melt%>%
#   ungroup()%>%
#   mutate(ymin = pmin(value, avg_cost),
#          ymax = pmax(value, avg_cost))
# 
# cost_desc_graph = gsub("\\n","\n",p2_p3_cost_sensitivity_tornado_plot_df_melt2$cost_desc_graph,fixed=T)
# 
# tornado_plot_cost_sensitivity<-ggplot() + 
#   geom_rect(data = p2_p3_cost_sensitivity_tornado_plot_df_melt2,
#             aes(ymax=ymax, 
#                 ymin=ymin, 
#                 xmax=xmax, 
#                 xmin=xmin, 
...
#   theme(axis.title.y=element_blank(), legend.position = 'bottom',
#         legend.title = element_blank()) + 
#   geom_hline(yintercept = p3_p2_avg_cost$est)+
#   geom_hline(yintercept = 0, linetype=2, color = "grey")+
#   scale_x_continuous(breaks = c(1:length(p2_p3_cost_sensitivity_tornado_plot_df_melt2$cost_desc_graph)), 
#                      labels = cost_desc_graph) +
#   coord_flip()+
...
#calculate incremental costs by year
incremental_costs_by_year_undisc<-cost_itemize_by_year_df%>%
  group_by(program_id, year, sim_id)%>%
  summarise(undisc_cost_itemized = sum(undisc_cost_itemized))%>%
  filter(program_id != 1)%>%
  mutate(undisc_cost_itemized = if_else(program_id == 2, 
                                        -undisc_cost_itemized, 
                                        undisc_cost_itemized))%>%
  group_by(year, sim_id)%>%
  summarise(incremental_undisc_cost = sum(undisc_cost_itemized))%>%
  group_by(year)%>%
  summarise(mean_incremental_undisc_cost = mean(incremental_undisc_cost))

######write####
setwd(outdir_results)
write.csv(incremental_ICER_calcs_df_exp_cost_mean_max_min,
          'DALY_cost_ICER_summarised.csv',
          row.names = FALSE)
write.csv(mort_total_df,
          'mort_total_df.csv',
          row.names = FALSE)
write.csv(inc_total_df,
          'inc_total.csv',
          row.names = FALSE)
write.csv(cost_itemize_by_year_df,
          'cost_itemize_by_year_df.csv',
          row.names = FALSE)
write.csv(n_ipt_active_DSTB_df,
          'n_ipt_active_DSTB_df.csv',
          row.names = FALSE)
write.csv(incremental_costs_by_year_undisc,
          'incremental_costs_by_year_undisc.csv',
          row.names = FALSE)
write.csv(recent_remote_df, 'recent_remote_df.csv', row.names = FALSE)

png(file="tornado_plot_cost_sensitivity_p3_p1_disc_DALYS.png",
    width = 8.5, height = 6, units = "in", res = 1000)


print(tornado_plot_cost_sensitivity_p3_p1_disc_DALYS)
dev.off()
