...
for (i in 1:length(all_files_in_outdir)){
  ...
  for (s in unique(cost_sensitivity_scenarios_df$sensitivity_id)){
    ...
    cost_adj_temp<-if_else(sensitivity_desc_temp == "upper", 1.25, 0.75)
...
width <- 1
type<-c("Annual outpatient HIV care cost, on ART\nunder community-based ART (Programs 2 and 3)\n($233-$388)",
        "Annual outpatient HIV care cost, on ART\nunder facility-based ART (Program 1)\n($187-$311)",
        "Annual outpatient HIV care cost, not on ART\n($101-$169)",
        "Cost of a course of TB treatment for people with DS-TB\n($97-$161)",
        "Annual inpatient HIV care costs, on ART\n($61-$101)",
        "Annual inpatient HIV care costs, not on ART, CD4 ≤ 200\n($65-$109)",
        #"Annual inpatient HIV care costs, not on ART, CD4 <= 200\n($65-$109)",
        "Cost of a course of TB treatment for people with MDR-TB\n($1,417-$2,361)",
        "Annual inpatient HIV care costs, not on ART, CD4 > 200\n($26-$43)",
        "6-month course of IPT\n($15-$25)"
        )
...
ggplot() + 
  ...
  scale_x_continuous(breaks = c(1:length(tornado_plot_df2$cost_desc)), 
                     labels = rep(rev(type),2))+
  scale_y_continuous(name="Discounted incremental cost per DALY averted") +
  coord_flip()

#nipt
n_ipt_df_by_community_v_facility_total<-n_ipt_df%>%
...
n_ipt_df_by_community_v_facility_overtime<-n_ipt_df%>%
  mutate(program_group = if_else(program_id == 3, "Community-based IPT delivery\n(Program 3)", "Facility-based IPT delivery\n(Programs 1 and 2)"))%>%
  mutate(value_avg = if_else(program_id == 3, n_ipt, n_ipt/2))%>%
  group_by(sim_id, program_group, time)%>%
  summarise(n_ipt = sum(value_avg))%>%
  group_by(program_group, time)%>%
  summarise(mean = mean(n_ipt),
            min = min(n_ipt),
            max = max(n_ipt))%>%
  mutate(time = time + 2018)

ggplot(n_ipt_df_by_community_v_facility_overtime, 
       aes(x=time, y=mean, group=program_group)) +
  geom_line(aes(linetype=program_group, color =program_group))+
  #geom_point(aes(shape=program_group, color =program_group))+
  ...
  scale_x_continuous(breaks=seq(2018,2028,2))+
  scale_y_continuous(breaks=seq(0,2500,500), name = "IPT initiation, per 100,000 individuals")
