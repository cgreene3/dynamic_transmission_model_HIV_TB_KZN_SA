...
#1940 to end of 2017 under program 1
#2017 to 2028 under each program
...
outdir<-'/gscratch/icrc/cgreene3/program_run_outputs/'
...
#location where want outputs
outdir<-paste0(here(), '/model_execution/program_runs/test_scripts_outputs')
...
sim_id_current_eval<-as.integer(sim_calibration_ref_df$sim_id[args_temp])
...
ipt_initiation_function<-function(yr){
  ...
  for (g in G_SET){
    ...
    #print('current policy eval')
    #print(current_policy_eval)
    #print(IPT_init_temp)
  }
  return(kappa_t_h_g)
}
  ...
  for (g in G_SET){
    ...
    hiv_incidence_temp<-hiv_incidence_df%>%
      filter(sex == if_else(g == 1, 'Male', 'Female'),
             year == yr, policy_id == current_policy_eval) #current_policy_eval set to 1 during warmup
      
      eta_i_h_g[1,2,g]<-as.numeric(hiv_incidence_factor_g[g]*hiv_incidence_temp$val[1])
    ...
    if(yr >= 2004){
      ...
      if(yr <= 2017){
        art_coverage_temp<-art_coverage_df%>%
          filter(sex == if_else(g == 1, 'Male', 'Female'),
                 year == yr,
                 policy_id == current_policy_eval) #current_policy_eval is set to 1
      } else {
        art_coverage_temp<-art_coverage_df%>%
          filter(sex == if_else(g == 1, 'Male', 'Female'),
                 year == 2018,
...
mort_param_func <-function(yr){
  ...
  for (g in G_SET){
    ...
    if(yr < 1990){
      ...
      mort_base_temp <- as.numeric(mu_baseline_factor_g[g]*mort_base_temp$val[1])
...
#warmup period#
#set current policy eval to 1 over warmup period
current_policy_eval <-1
start_yr_temp<-start_yr_warmup #for year calculations
current_yr <- start_yr_warmup #tracks for dynamic params
last_yr<-start_yr_warmup-1 #the year before warmup period


out_df_warmup<-as.data.frame(ode(times = TT_SET_warmup, y = N_init,
                          func = tb_hiv_prog_calibration_model, method = 'lsoda',
                          parms = NULL))

out_df_warmup<- cbind(year = as.integer(start_yr_warmup+out_df_warmup$time), sim_id = rep(sim_id_current_eval,
                                                                                    times = nrow(out_df_warmup)),
                      out_df_warmup)

#policy
N_init_start_of_eval<-out_df_warmup%>%
  filter(time == max(time))%>%
  select(-c('time', 'year', 'sim_id'))

start_yr_temp<-start_yr_eval
current_yr <- start_yr_eval-1 #tracks for dynamic params
POLICY_SET<-1:3

out_df_eval_all<-data.frame()
summarised_eval_metrics_df_all<-data.frame()

for (p in POLICY_SET){
  current_policy_eval <-p
  start_yr_temp<-start_yr_eval #for year calculations
  current_yr <- start_yr_eval #tracks for dynamic params
  last_yr<-start_yr_eval-1 #the year before warmup period updates
  
  out_df_eval<-as.data.frame(ode(times = TT_SET_eval, y = unlist(N_init_start_of_eval),
                            func = tb_hiv_prog_calibration_model, method = 'lsoda',
                            parms = NULL))
  
  out_df_eval<- cbind(year = as.integer(start_yr_eval+out_df_eval$time), 
                      sim_id = rep(sim_id_current_eval,
                                   times = nrow(out_df_eval)),
                      out_df_eval)
  
  out_df_eval<-rbind(out_df_warmup, out_df_eval)
  out_df_eval<- cbind(program_id = rep(current_policy_eval,
                                   times = nrow(out_df_eval)),
                      out_df_eval)
  
  #calculate population estimates
  out_df_eval$total_male_pop<-rowSums(out_df_eval[,3+c(N_t_r_h_g_ref[TB_SET, DR_SET, HIV_SET,1])])
  out_df_eval$total_female_pop<-rowSums(out_df_eval[,3+c(N_t_r_h_g_ref[TB_SET, DR_SET, HIV_SET,2])])
  out_df_eval$hiv_prev_male<-rowSums(out_df_eval[,3+c(N_t_r_h_g_ref[TB_SET, DR_SET, c(2,3,4),1])]) 
  out_df_eval$hiv_prev_female<-rowSums(out_df_eval[,3+c(N_t_r_h_g_ref[TB_SET, DR_SET, c(2,3,4),2])])
  
  #calculate rates per 100K people
  out_df_eval$tb_mort_t_cumulative<-out_df_eval$Tb_mort_neg_female+
    out_df_eval$Tb_mort_neg_male+
    out_df_eval$Tb_mort_pos_female+
    out_df_eval$Tb_mort_pos_male
  
  out_df_eval$omort_t_cumulative<-out_df_eval$O_mort_female+
    out_df_eval$O_mort_male
  
  out_df_eval$tb_inc_t_cumulative<-out_df_eval$Tb_inc_neg_female+
    out_df_eval$Tb_inc_neg_male+
    out_df_eval$Tb_inc_pos_female+
    out_df_eval$Tb_inc_pos_male
  
  out_df_eval$hiv_prev_t<-out_df_eval$hiv_prev_male+out_df_eval$hiv_prev_female
  
  out_df_eval$n_people_on_ART_t<-rowSums(out_df_eval[,3+c(N_t_r_h_g_ref[TB_SET, DR_SET,4,G_SET])])

  #######summarise calibration metrics######
  subset_out_df<-out_df_eval%>%
    select(c('year', 'time', 
             model_output_names,
             'total_female_pop',
             'total_male_pop'))
  
  summarised_metrics_TB<-out_df_eval%>%
    mutate(Tb_inc_neg_female_t_per_100k_ppl = Tb_inc_neg_female-lag(Tb_inc_neg_female),
           Tb_inc_neg_male_t_per_100k_ppl = Tb_inc_neg_male-lag(Tb_inc_neg_male),
           Tb_inc_pos_female_t_per_100k_ppl = Tb_inc_pos_female-lag(Tb_inc_pos_female),
           Tb_inc_pos_male_t_per_100k_ppl = Tb_inc_pos_male-lag(Tb_inc_pos_male),
           Tb_mort_neg_female_t_per_100k_ppl = Tb_mort_neg_female-lag(Tb_mort_neg_female),
           Tb_mort_neg_male_t_per_100k_ppl = Tb_mort_neg_male-lag(Tb_mort_neg_male),
           Tb_mort_pos_female_t_per_100k_ppl = Tb_mort_pos_female-lag(Tb_mort_pos_female),
           Tb_mort_pos_male_t_per_100k_ppl = Tb_mort_pos_male-lag(Tb_mort_pos_male))%>%
    mutate(Tb_inc_neg_female_t_per_100k_female = ((Tb_inc_neg_female_t_per_100k_ppl/total_female_pop)*100000),
           Tb_inc_neg_male_t_per_100k_male = ((Tb_inc_neg_male_t_per_100k_ppl/total_male_pop)*100000),
           Tb_inc_pos_female_t_per_100k_female = ((Tb_inc_pos_female_t_per_100k_ppl/total_female_pop)*100000),
           Tb_inc_pos_male_t_per_100k_male = ((Tb_inc_pos_male_t_per_100k_ppl/total_male_pop)*100000),
           Tb_mort_neg_female_t_per_100k_female = ((Tb_mort_neg_female_t_per_100k_ppl/total_female_pop)*100000),
           Tb_mort_neg_male_t_per_100k_male = ((Tb_mort_neg_male_t_per_100k_ppl/total_male_pop)*100000), 
           Tb_mort_pos_female_t_per_100k_female = ((Tb_mort_pos_female_t_per_100k_ppl/total_female_pop)*100000), 
           Tb_mort_pos_male_t_per_100k_male = ((Tb_mort_pos_male_t_per_100k_ppl/total_male_pop)*100000))%>%
    group_by(year)%>%
    summarise(Tb_inc_neg_female_Y_per_100k_female = sum(Tb_inc_neg_female_t_per_100k_female),
              Tb_inc_neg_male_Y_per_100k_male = sum(Tb_inc_neg_male_t_per_100k_male),
              Tb_inc_pos_female_Y_per_100k_female = sum(Tb_inc_pos_female_t_per_100k_female),
              Tb_inc_pos_male_Y_per_100k_male = sum(Tb_inc_pos_male_t_per_100k_male),
              Tb_mort_neg_female_Y_per_100k_female = sum(Tb_mort_neg_female_t_per_100k_female),
              Tb_mort_neg_male_Y_per_100k_male = sum(Tb_mort_neg_male_t_per_100k_male), 
  ...
  summarised_metrics_hiv_prev<-out_df_eval%>%
  ...
  summarised_eval_metrics_df<-summarised_metrics_TB%>%
    ...
    mutate(sim_id = sim_id_current_eval,
           program_id = current_policy_eval)
  
  if(p == 1){
    summarised_eval_metrics_df_all<-summarised_eval_metrics_df%>%
      filter(year <= 2028)
    out_df_eval_all<-out_df_eval%>%
      filter(year >= 2017)
  } else {
    ...
    out_df_eval_all<-rbind(out_df_eval_all, out_df_eval%>%filter(year >= 2017))
  }
}

setwd(outdir)
write.csv(summarised_eval_metrics_df_all, 
          file = paste0('summarised_eval_metrics_df_all_sim_id_', sim_id_current_eval, '.csv'), 
          row.names = FALSE)

setwd(paste0(outdir, '/df_for_economic_analysis'))
write.csv(out_df_eval_all, paste0('out_df_sim_id_', sim_id_current_eval, '.csv'), row.names = FALSE)
