...
n_accepted_param_sets<-1708  #number of accepted parameter sets
n_sims_in_loop<-2 #number of sims run each loop
...
varepsilon_g_param_extraction<-function(sim_id_current_eval){
  ...
  for (g in G_SET){
    ...
    temp_sim_val<-sim_calibration_ref_df%>%
      filter(sim_id == sim_id_current_eval)%>%
      select("varepsilon_") #no longer differentiated by gender
  ...
  for (g in G_SET){
    hiv_incidence_temp<-hiv_incidence_df%>%
      filter(sex == if_else(g == 1, 'Male', 'Female'),
             year == yr, program_id == current_program_eval)
    ...
    if(yr >= 2004){
      ...
      if(yr <= 2017){
        art_coverage_temp<-art_coverage_df%>%
          filter(sex == if_else(g == 1, 'Male', 'Female'),
                 year == yr,
                 program_id == current_program_eval) #current_program_eval is set to 1
      } else {
        art_coverage_temp<-art_coverage_df%>%
          filter(sex == if_else(g == 1, 'Male', 'Female'),
                 year == 2018,
...
for(sim_id_current_eval in all_sim_ids_in_current_eval){  
  ...
  model_output_names<-c('Tb_inc_neg_male_cumulative',
                        'Tb_inc_neg_female_cumulative',
                        'Tb_inc_pos_male_cumulative',
                        'Tb_inc_pos_female_cumulative',
                        'Tb_mort_neg_male_cumulative',
                        'Tb_mort_neg_female_cumulative',
                        'Tb_mort_pos_male_cumulative',
                        'Tb_mort_pos_female_cumulative',
                        'O_mort_male_cumulative',
                        'O_mort_female_cumulative',
                        'IPT_init_cumulative')
  ...
  out_df_prog_eval<-rbind(out_df_temp_warmup,
                                  out_df_temp_eval)
  
  #calculate population estimates
  out_df_prog_eval$total_t_male_pop<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, HIV_SET,1])])
  out_df_prog_eval$total_t_female_pop<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, HIV_SET,2])])
  
  #calculate hiv prev
  out_df_prog_eval$hiv_prev_t_male_per_100K_ppl<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, c(2,3,4),1])]) 
  out_df_prog_eval$hiv_prev_t_female_per_100K_ppl<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, c(2,3,4),2])])
  
  #calculate art coverage
  out_df_prog_eval$n_on_art_t_per_100K_ppl<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[TB_SET, DR_SET, 4, G_SET])])
  
  #calculate TB prev
  out_df_prog_eval$tb_prev_t_per_100K_ppl<-rowSums(out_df_prog_eval[,4+c(N_t_r_h_g_ref[6, DR_SET, HIV_SET, G_SET])])
  

  #######summarise calibration metrics######
  subset_out_df<-out_df_prog_eval%>%
    select(c('year', 'time', 
             model_output_names,
             'total_t_male_pop',
             'total_t_female_pop',
             'hiv_prev_t_male_per_100K_ppl',
             'hiv_prev_t_female_per_100K_ppl',
             'tb_prev_t_per_100K_ppl'))
  ...
  summarised_metrics_TB_by_gender<-subset_out_df%>%
    mutate(Tb_inc_neg_female_t_per_100k_ppl = Tb_inc_neg_female_cumulative-lag(Tb_inc_neg_female_cumulative),
           ...
           Tb_mort_neg_female_t_per_100k_ppl = Tb_mort_neg_female_cumulative-lag(Tb_mort_neg_female_cumulative),
    ...
    mutate(Tb_inc_neg_female_t_per_100k_female = ((Tb_inc_neg_female_t_per_100k_ppl/total_t_female_pop)*100000),
           Tb_inc_neg_male_t_per_100k_male = ((Tb_inc_neg_male_t_per_100k_ppl/total_t_male_pop)*100000),
           Tb_inc_pos_female_t_per_100k_female = ((Tb_inc_pos_female_t_per_100k_ppl/total_t_female_pop)*100000),
           Tb_inc_pos_male_t_per_100k_male = ((Tb_inc_pos_male_t_per_100k_ppl/total_t_male_pop)*100000),
           Tb_mort_neg_female_t_per_100k_female = ((Tb_mort_neg_female_t_per_100k_ppl/total_t_female_pop)*100000),
           Tb_mort_neg_male_t_per_100k_male = ((Tb_mort_neg_male_t_per_100k_ppl/total_t_male_pop)*100000), 
           Tb_mort_pos_female_t_per_100k_female = ((Tb_mort_pos_female_t_per_100k_ppl/total_t_female_pop)*100000), 
           Tb_mort_pos_male_t_per_100k_male = ((Tb_mort_pos_male_t_per_100k_ppl/total_t_male_pop)*100000),
           Tb_inc_neg_t_per_100k_ppl = sum(Tb_inc_neg_male_t_per_100k_ppl, Tb_inc_neg_female_t_per_100k_ppl),
           Tb_inc_pos_t_per_100k_ppl = sum(Tb_inc_pos_female_t_per_100k_ppl, Tb_inc_pos_male_t_per_100k_ppl),
           Tb_mort_neg_t_per_100k_ppl = sum(Tb_mort_neg_male_t_per_100k_ppl, Tb_mort_neg_female_t_per_100k_ppl),
           Tb_mort_pos_t_per_100k_ppl = sum(Tb_mort_pos_male_t_per_100k_ppl, Tb_mort_pos_male_t_per_100k_ppl))%>%
    group_by(year)%>%
    summarise(Tb_inc_neg_female_Y_per_100k_female = sum(Tb_inc_neg_female_t_per_100k_female),
              ...
              Tb_mort_neg_Y_per_100k_ppl = sum(Tb_mort_neg_t_per_100k_ppl),
              Tb_mort_pos_Y_per_100k_ppl = sum(Tb_mort_pos_t_per_100k_ppl))%>%
    filter(year >= 1990)%>%
    filter(year <= 2027)
  
  summarised_metrics_hiv_prev_by_gender<-subset_out_df%>%
    mutate(hiv_prev_per_100k_female = ((hiv_prev_t_female_per_100K_ppl/total_t_female_pop)*100000),
           hiv_prev_per_100k_male = ((hiv_prev_t_male_per_100K_ppl/total_t_male_pop)*100000))%>%
    group_by(year)%>%
   ...
   summarised_metrics_tb_prev<-subset_out_df%>%
    group_by(year)%>%
    summarise(tb_prev_Y_per_100K_ppl = mean(tb_prev_t_per_100K_ppl))%>%
    filter(year >= 1990)%>%
    filter(year <= 2027)
  ...
  if(current_program_eval == 1){
    ...
    out_df_eval_all<-rbind(out_df_eval_all, 
                           out_df_prog_eval%>%filter(year >= 2017))
  }
  }
  setwd(outdir)
  write.csv(summarised_eval_metrics_df_all, 
            paste0('summarised_eval_metrics_df_all_sim_id_', 
                   sim_id_current_eval_temp,
                   '.csv'),
            row.names = FALSE)
  setwd(outdir_economic_analysis)
  write.csv(out_df_eval_all, 
            paste0('out_df_eval_all_sim_id_', 
                   sim_id_current_eval_temp,
                   '.csv'),
            row.names = FALSE)
}
