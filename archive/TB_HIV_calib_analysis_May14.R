...
indir_sim_ref<-paste0(here(),'/model_outputs/calibration_data_sets/', current_date)
...
calibration_df_all<-subset(calibration_df_all, select = -V1)
...
lapply(subset_list, function(s){
  ...
  print(Sys.time())
  upper_sim <-s
  lower_sim <-max(0, upper_sim-243)
  
  calibration_df_all_temp<-calibration_df_all%>%
    filter(sim_id < upper_sim)%>%
    filter(sim_id >= lower_sim)
...
top_graphs_func<-function(s_top){
  calibration_df_all_top<-calibration_df_all%>%
    filter(sim_id %in% s_top)
