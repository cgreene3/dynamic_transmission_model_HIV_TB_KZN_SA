...
mort_calib_TB_only_test <- 1:2
mort_calib_TB_HIV_CD4More_test <- 1:2
mort_calib_TB_HIV_CD4Less_test <- 1:2
mort_calib_TB_HIV_ART_test <- 1:2
...
#filtered df according to current mortality evaluated#
mort_df_calib<-data.frame()
...
tb_hiv_prog_model <- function(time, N_t_r_h_g, parms){
...
for(row in 1:nrow(sim_calibration_ref_df)){
  
  #to track how long it takes to solve each run
  start<-Sys.time()
  
  #pull calibration specific data
  #mort specific data
  mort_df_calib<-mort_df%>%
    filter(mu1 == m1_calib_id,
           mu2 == m2_calib_id,
           mu3 == m3_calib_id,
           mu4 == m4_calib_id)
  
  #beta specific data
  beta_g[1]<-sim_calibration_ref_df[row, "b1_calib_id"]
  beta_g[2]<-sim_calibration_ref_df[row, "b2_calib_id"]
  
  #pop init (by betas)
  pop_init_df_temp<-pop_init_df%>%
    filter(beta_1 == beta_g[1],
           beta_2 == beta_g[2])
  ...
  out_df<-as.data.frame(ode(times = TT_SET, y = N_init,
                            func = tb_hiv_prog_model, method = 'lsoda',
                            parms = NULL))
  
  sim_id<-sim_calibration_ref_df[row, "sim_calib_id"]
  
  out_df<- cbind(year = as.integer(start_yr+out_df$time), sim_id = rep(sim_id, times = nrow(out_df)),
                 out_df)
  
  write.csv(out_df, file = paste0('calib_sim_', sim_id, '.csv'))
