#Policy Analysis Dec 21 to 2028
...
indir_state_prog<-paste0(here(), '/policy_runs/results/', start_eval_date)
outdir_analysis <- paste0(here(), '/policy_runs/results/', start_eval_date, 'analysis')
...
vis_TB_mort<-function(mse_df_top){
  ...
  for (g in c(1,2)){
    ...
    for (hs in c('negative', 'positive')){
      ...
      max_graph = if_else(hs == 'negative', 150, 450)
