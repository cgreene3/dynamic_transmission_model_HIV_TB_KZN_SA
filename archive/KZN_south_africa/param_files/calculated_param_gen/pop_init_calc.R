...
#!!!Make sure to run all other param calc ##
##scripts before running pop init calc ##
...
outdir_indir <- paste0(here(),'/param_files')
indir_pop<-paste0(here(), '/param_files/calculated_param_gen/input_data')
...
#read in data
mort_rate_df<-read.csv('KZN_mort_rate_df.csv')
birth_perc_df<-read.csv('birth_perc_df.csv')
hiv_incidence_df<-read.csv('hiv_incidence_df.csv')
...
mort_rate_df$G_compartment<-as.integer(mort_rate_df$sex_id)
...
#no IPT initiation assumed in warmup period
...
HIV_transitions_param_func<-function(yr){
  ...
  for (g in G_SET){
    ...
    temp2<-hiv_incidence_df%>%
      filter(gender_id == g,
             year == yr)
    eta_i_h_g[1,2,g]<-temp2$hiv_incidence 
    #NO ART in warmup period
    }
  return(eta_i_h_g)
  }
...
mort_df_base_g<-mort_rate_df%>%
  filter(year == 1990)%>%
  arrange(sex_id)

for (t in TB_SET){
  for (h in HIV_SET)
  mu_t_h_g_base[t,h,G_SET]<-mort_df_base_g$mort_rate
}
...
names() <- c(pop_init_df$compartment_id)
...
for(n in sim_calibration_ref_df$sim_calib_id){
    ...
    #update hiv mort rates according to current calibration
    ...
    #update tb/hiv mort rates according to current calibration
    ...
    out_df<-as.data.frame(ode(times = TT_SET, y = , 
