...
#make sure environment is set to KZN_south_africa for here () to work
...
outdir_mse_dfs<-paste0(here(),'/param_files/calibration_code_results/', start_eval_date)
outdir_analysis <- paste0(here(),'/param_files/calibration_code_results/', start_eval_date, '/analysis')
dir.create(outdir_mse_dfs)
...
calibration_weights_df<-read.csv('calibration_year_weights.csv')
...
lapply(subset_list, function(s){
  ...
  #weigh later years more than earlier years
  
  
  mse_df_temp<-mort_calibration_df_all%>%
    mutate(diff = (mort_rate_per_100K - expected_rate),
           mse = diff^2)%>%
    mutate(mse_weighted = weight*mse)%>%
    group_by(sim_id, calibration_group)%>%
    summarise(calibration_group_mse = sum(mse_weighted))%>%
    ungroup()
...
mse_df2<-mse_df2%>%
  ...
  mutate(hiv_prev_mse_male_norm_rank = rank(hiv_prev_mse_male_norm),
         hiv_prev_mse_female_norm_rank = rank(hiv_prev_mse_female_norm))
  

setwd(outdir_mse_dfs)
write.csv(mse_df2, 'mse_df.csv', row.names = FALSE)

############Determine hiv prev cut off###########
# ggplot(mse_df2%>%arrange(hiv_prev_mse_male_norm_rank),
#        aes(x=hiv_prev_mse_male_norm_rank, y=hiv_prev_mse_male_norm)) +
#   geom_line()
# 
# ggplot(mse_df2%>%arrange(hiv_prev_mse_female_norm_rank),
#        aes(x=hiv_prev_mse_female_norm_rank, y=hiv_prev_mse_female_norm)) +
#   geom_line()

##filter on appropriate hiv prev norm factor

mse_df3<-mse_df2%>%
  filter(hiv_prev_mse_male_norm <= .6,
         hiv_prev_mse_female_norm <= .1)%>%
  ...
  mutate(tb_mort_mse_overall_norm = ((.25*tb_mort_mse_male_neg_norm)+
                                       (.25*tb_mort_mse_female_neg_norm)+
                                       (.25*tb_mort_mse_male_pos_norm)+
                                       (.25*tb_mort_mse_female_pos_norm)))%>%
...
write.csv(mse_df3, 'mse_df_filtered.csv', row.names = FALSE)
...
top_graphs_func<-function(mse_df_top){
  ...
  for (sim_id_itr in mse_df_top$sim_id){
    ...
    ART_coverage_df<-state_prog_df%>%
      ...
      group_by(year, G_compartment)%>%
      summarise(TB_HIV_model_estimates = median(ART_coverage))%>%
      mutate(G_compartment = as.integer(G_compartment))%>%
      left_join(ART_df%>%
                  select(c('year', 'G_compartment', 'art_coverage')), 
                by = c('year', 'G_compartment'))%>%
      rename(HPV_HIV_model_estimates = art_coverage)
  ...
  for (s in mse_df_top$sim_id){
    ...
    for (g in unique(mort_calibration_df_all_temp$G_compartment)){
      ...
      graph_temp <- ggplot()+
        ...
        scale_y_continuous(name = 'mortality rate', limits = c(0, 1000), breaks=(seq(0, 1000, 100)))+
...
mse_df_top_tb_mort_hiv_neg_male<-mse_df3%>%
...
mse_df_top_tb_mort_hiv_neg_female<-mse_df3%>%
...
mse_df_top_tb_mort_hiv_pos_male<-mse_df3%>%
...
mse_df_top_tb_mort_hiv_pos_female<-mse_df3%>%
