...
open_seir_model <- function(time, N_t_h, parms){
  ...
  FOI <- (beta*(sum((1/phi_h)*N_t_h[N_t_h_ref[6, HIV_SET]])/sum(N_t_h)))
  ...
  lapply(HIV_SET, function(h){
    dN_t_h[N_t_h_ref[1,h]]<<-((alpha_in_t_h[1,h]*B) - #entries from births
                                ...
                                (sum(eta_i_h[HIV_SET, h]*N_t_h[N_t_h_ref[1,HIV_SET]])) - #entries into HIV compartment (no TB transition)
                                (sum(eta_i_h[h,HIV_SET])*N_t_h[N_t_h_ref[1,h]]) + #exit from HIV compartment (no TB transition)
                                (omega*N_t_h[N_t_h_ref[2,h]]) - #entries from off IPT (no HIV transition)
                                (FOI*N_t_h[N_t_h_ref[1,h]])- #exists from TB infection (no HIV transition)
                                (kappa_t_h[1,h]*N_t_h[N_t_h_ref[1,h]]) + #exists from on to IPT (no HIV transition)
                                (omega*eta_i_h[HIV_SET, h]*N_t_h[N_t_h_ref[2,HIV_SET]]) - #entries from off IPT and HIV transition
                                (FOI*sum(eta_i_h[h,HIV_SET])*N_t_h[N_t_h_ref[1,h]]) - #exists from TB infection and HIV transition
                                (kappa_t_h[1,HIV_SET]*sum(eta_i_h[h,HIV_SET])*N_t_h[N_t_h_ref[1,h]]) #exists from on to IPT (and HIV transition)
    )
  ...
  #TB compartment 2
  lapply(HIV_SET, function(h){
    dN_t_h[N_t_h_ref[2,h]]<<-((total_out_t_h[N_t_h_ref[2,h]]*N_t_h[N_t_h_ref[2,h]])- #exists from aging out and death
                                ...
                                (kappa_t_h[1,h]*N_t_h[N_t_h_ref[1,h]])- #entries from on to IPT (no HIV transition)
                                (omega*N_t_h[N_t_h_ref[2,h]])- #exits from off IPT (no HIV transition)
                                (iota*FOI*N_t_h[N_t_h_ref[2,h]]) + #exits from infection (diminished for IPT, no HIV transition
                                (sum(kappa_t_h[1,HIV_SET]*eta_i_h[HIV_SET, h]*N_t_h[N_t_h_ref[1,HIV_SET]])) - #entries from onto IPT and HIV transition
                                (omega*sum(eta_i_h[h,HIV_SET])*N_t_h[N_t_h_ref[2,h]]) - #exists from IPT and HIV transition
                                (iota*FOI*sum(eta_i_h[h,HIV_SET])*N_t_h[N_t_h_ref[2,h]])
    )
  })
