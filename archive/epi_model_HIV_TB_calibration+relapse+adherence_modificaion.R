...
#########santity checks#########
outdir_sanity_check <- paste0(here(),'/model_outputs/sanity_check/Mar22')
setwd(outdir_sanity_check)

TB_all_overtime<-out_df_melt%>%
  group_by(TB_compartment, time)%>%
  summarise(total_in_compartment = sum(value))%>%
  filter(TB_compartment != 'pop')

tb_all_graph<-ggplot(data = TB_all_overtime, mapping = aes(x = time, 
                                             y = total_in_compartment, 
                                             color = TB_compartment))+
  geom_line()+
  labs(title = 'TB compartments overtime warmup')

png("tb_all_overtime_warmup.png")
print(tb_all_graph)
dev.off()

TB_prev_overtime<-out_df_melt%>%
  mutate(HIV_group = if_else(HIV_compartment == 1, 'HIV-', 'HIV+'),
         G_group = if_else(G_compartment == 1, 'Male', 'Female'),
         HIV_G_group = paste0(G_group,', ', HIV_group))%>%
  group_by(time, G_compartment)%>%
  mutate(total_gender = sum(value))%>%
  ungroup()%>%
  filter(TB_compartment == 6)%>%
  group_by(time, HIV_G_group, total_gender)%>%
  summarise(TB_total_in_group = sum(value))%>%
  ungroup()%>%
  mutate(TB_percent = TB_total_in_group/total_gender)%>%
  mutate(TB_prev = TB_percent*100000,
         time = time + start_yr)

active_tb_graph<-ggplot(data = TB_overtime, mapping = aes(x = time, y = TB_prev))+
  geom_line(aes(colour = HIV_G_group), size = 1)+
  labs(title = 'TB prevalence, warmup')+
  scale_y_continuous(name="rate", limits=c(0, 2400))+
  scale_x_continuous(name = 'time', breaks=seq(from = 1990, to = 2017, by = 5))+
  scale_color_manual(values=c("red", "#56B4E9", "purple", "green"))

png("activeTB_warmup.png")
print(active_tb_graph)
dev.off()
# 
