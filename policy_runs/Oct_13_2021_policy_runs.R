...
#and code see param_files/calibration_code_results/2021_09_22/analysis 
...
hiv_transition_df<-read_excel('hiv_transmission_df.xlsx', sheet = 'warm_up')
...
HIV_transitions_param_func<-function(yr, N_t_r_h_g){
  ...
  for (g in G_SET){
    ...
    if(yr >= 2004){
      ...
      #if before calibration years
      if (yr < 2017){
        n4_prop_next_yr <- as.double(hiv_transition_df$art_coverage[nxt_yr_row_temp])
      }
      else {
