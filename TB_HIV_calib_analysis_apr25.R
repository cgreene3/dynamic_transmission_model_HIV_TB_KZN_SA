...
indir <- paste0(here(),'/model_outputs/calibration/', date)
outdir <- paste0(here(),'/model_outputs/calibration/', date, '/analysis')
...
state_prog_df <-melt(calibration_df_all, id.vars = c("time", "year",
                                               'sim_id'))
...
mort_calibration_df<-calibration_df_all%>%
  ...
  as.data.frame()
# 
mort_calibration_df<-melt(mort_calibration_df, id = c('year', 'sim_id'))
...
for (g in unique(mort_calibration_df_all$sex_name)){
  ...
  df_temp <- mort_calibration_df_all%>%filter(sex_name == g, sim_id == 95)
