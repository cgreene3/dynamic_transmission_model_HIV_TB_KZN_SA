...
setwd(outdir) #outdir is where mort df is stored
mort_df<-read.csv('mort_df.csv')
h2_ONLY_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 2, 
                                                      TB_compartment != 6),
                                     mort_adj))))
h3_ONLY_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 3,
                                                      TB_compartment != 6),
                                     mort_adj))))
h4_ONLY_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 4,
                                                      TB_compartment != 6),
                                     mort_adj))))
TB_ONLY_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 1, 
                                                      TB_compartment == 6),
                                     mort_adj))))
TB_HIV2_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 2, 
                                                      TB_compartment == 6),
                                     mort_adj))))
TB_HIV3_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 3, 
                                                      TB_compartment == 6),
                                     mort_adj))))
TB_HIV4_test<-unlist(c(unique(select(mort_df%>%filter(HIV_compartment == 4, 
                                                      TB_compartment == 6),
                                     mort_adj))))

#betas to test
beta_1_test<-seq(from = 6, to = 11, by = .8)
beta_2_test<-seq(from = 6, to = 11, by = .8)
...
for (b1 in beta_1_test){
  for (b2 in beta_2_test){
    for (mh2 in h2_ONLY_test){
      for (mh3 in h3_ONLY_test){
        for (mh4 in h4_ONLY_test){
          for (mtb in TB_ONLY_test){
            for (mtb_h2 in TB_HIV2_test){
              for (mtb_h3 in TB_HIV3_test){
                for (mtb_h4 in TB_HIV4_test){
              
                  sim_calib_id<-c(sim_calib_id, sim_id_temp)
                  b1_calib_id<-c(b1_calib_id, b1)
                  b2_calib_id<-c(b2_calib_id, b2)
                  mh2_calib_id<-c(mh2_calib_id, mh2)
                  mh3_calib_id<-c(mh3_calib_id, mh3)
                  mh4_calib_id<-c(mh4_calib_id, mh4)
                  mtb_calib_id<-c(mtb_calib_id, mtb)
                  mtbh2_calib_id<-c(mtbh2_calib_id, mtb_h2)
                  mtbh3_calib_id<-c(mtbh3_calib_id, mtb_h3)
                  mtbh4_calib_id<-c(mtbh4_calib_id, mtb_h4)
...
sim_calibration_ref_df<-data.frame(sim_calib_id, b1_calib_id, b2_calib_id,
                                   mh2_calib_id, mh3_calib_id, mh4_calib_id,
                                   mtb_calib_id, mtbh2_calib_id,
                                   mtbh3_calib_id, mtbh4_calib_id)
...
HIV_transitions_param_func<-function(yr){
  ...
  if (yr < 1980){
    ...
    for (g in G_SET){
      ...
      #eta_i_h_g[2,3,g]<-if_else(g == 1, (1/7.72), (1/9.5)) testing different rates of prog
...
mu_t_h_g_base <- array(0, dim = c(length(TB_SET), 
                                   length(HIV_SET), 
                                   length(G_SET)))

#set base mortality rates
mort_df_base_g<-mort_df%>%
  filter(year == 1990)%>%
  arrange(G_compartment)
mort_df_base_g<-c(unique(mort_df_base_g$mort_rate))

for (t in TB_SET){
  for (h in HIV_SET)
  mu_t_h_g_base[t,h,G_SET]<-mort_df_base_g
}

mu_t_h_g<-mu_t_h_g_base
...
total_out_param_func<-function(mu_t_h_g){
  count_temp <- 1
  total_out_t_r_h_g_temp <- array(0, 
                                  dim = length(TB_SET)*length(DR_SET)*length(HIV_SET)*length(G_SET))
  for (t in TB_SET){
    for (r in DR_SET){
      for (h in HIV_SET){
        for (g in G_SET){
          total_out_t_r_h_g_temp[count_temp] <- (mu_t_h_g[t,h,g]*(1-alpha_out))+
  ...
  total_out_t_r_h_g<<-total_out_t_r_h_g_temp
}
...
for(n in 1:nrow(sim_calibration_ref_df)){
    ...
    hiv_mort_rate_levels<-sim_attributes[4:6]
    tb_mort_rate_levels<-sim_attributes[7:10]
    
    #update mort rates according to current calibration
    mu_t_h_g[c(1:5, 7:8), 2, G_SET]<-mu_t_h_g_base[c(1:5, 7:8), 2,
                                                   G_SET]*unlist(hiv_mort_rate_levels[1])
    mu_t_h_g[c(1:5, 7:8), 3, G_SET]<-mu_t_h_g_base[c(1:5, 7:8), 3,
                                                   G_SET]*unlist(hiv_mort_rate_levels[2])
    mu_t_h_g[c(1:5, 7:8), 4, G_SET]<-mu_t_h_g_base[c(1:5, 7:8), 4,
                                                   G_SET]*unlist(hiv_mort_rate_levels[3])
    
    #update mort rates according to current calibration
    mu_t_h_g[6, 1, G_SET]<-mu_t_h_g_base[6, 1, G_SET]*unlist(tb_mort_rate_levels[1])
    mu_t_h_g[6, 2, G_SET]<-mu_t_h_g_base[6, 2, G_SET]*unlist(tb_mort_rate_levels[2])
    mu_t_h_g[6, 3, G_SET]<-mu_t_h_g_base[6, 3, G_SET]*unlist(tb_mort_rate_levels[3])
    mu_t_h_g[6, 4, G_SET]<-mu_t_h_g_base[6, 4, G_SET]*unlist(tb_mort_rate_levels[4])
    
    total_out_param_func(mu_t_h_g)
