...
hiv_transmission_df$eta_24<-rep(0, times = nrow(hiv_transmission_df))
hiv_transmission_df$eta_34<-rep(0, times = nrow(hiv_transmission_df))

for (yr in c(calib_years, eval_years)){
  for (g in genders){
    ...
    if(yr >= 2004){
      ...
      hiv_transmission_df$eta_24[current_yr_row_temp]<-art_inititation*n2_prop_eligible_current_yr
      hiv_transmission_df$eta_34[current_yr_row_temp]<-art_inititation
