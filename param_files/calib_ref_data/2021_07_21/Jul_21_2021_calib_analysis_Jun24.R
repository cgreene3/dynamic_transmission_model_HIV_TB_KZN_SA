#Calibration Analysis July 21th
...
description <- '1990_2018_ANALYSIS'
...
outdir_analysis <- paste0(here(),'/param_files/calib_ref_data/', start_eval_date, '/analysis/', description)
dir.create(paste0(here(),'/param_files/calib_ref_data/', start_eval_date, '/analysis/'))
...
mse_df2<-mse_df%>%
  left_join(sim_calibration_ref_df%>%
            ...
            by = c('sim_id'))%>%
  mutate(norm_hiv_prev_mse = ((hiv_prev_mse^(1/2))-min(hiv_prev_mse^(1/2)))/
           (max(hiv_prev_mse^(1/2))-min(hiv_prev_mse^(1/2))))%>%
  mutate(norm_tb_mort_mse = ((tb_mort_mse_total^(1/2))-min((tb_mort_mse_total^(1/2))))/
           (max(tb_mort_mse_total^(1/2))-min(tb_mort_mse_total^(1/2))))%>%
  mutate(norm_hiv_prev_tb_mort_mse = norm_hiv_prev_mse+norm_tb_mort_mse)%>%
  mutate(tbmort_rank_total = rank(tb_mort_mse_total),
         ...
         hiv_tb_rank = rank(norm_hiv_prev_tb_mort_mse))%>%
  as.data.frame()
...
top_graphs_func<-function(mse_df_top){
  ...
  for (sim_id_itr in mse_df_top$sim_id){
    ...
    temp_data <- fread(file_name) 
    temp_data$sim_id <- rep(sim_id_itr, times = nrow(temp_data))
    temp_data$hiv_tb_rank<-rep(as.integer(mse_df_top%>%
                                              filter(sim_id == sim_id_itr)%>%
                                              select(c(hiv_tb_rank))))
...
mse_df_top_tb_mort_hiv_prev<-mse_df2%>%
  filter(hiv_tb_rank<=10)
...
top_graphs_func(mse_df_top_tb_mort_hiv_prev)

setwd(outdir_best_analysis)
write.csv(mse_df_top_tb_mort_hiv_prev, paste0('mse_df_top_tb_hiv_', start_eval_date,
'_.csv'), row.names = FALSE)

#find best based on TB mort
mse_df_top_tb_mort<-mse_df2%>%
  filter(tbmort_rank_total<=10)

outdir_best_analysis<-paste0(outdir_analysis, '/tb_mort_best')
outdir_best_analysis_ART<-paste0(outdir_best_analysis, '/ART_COVERAGE')
outdir_best_analysis_HIV_PREV<-paste0(outdir_best_analysis, '/HIV_PREV')
outdir_best_analysis_TB_MORT<-paste0(outdir_best_analysis, '/TB_MORT')
...
setwd(outdir_best_analysis)
write.csv(mse_df_top_tb_mort, paste0('mse_df_top_tb_only_', start_eval_date,
                                              '_.csv'), row.names = FALSE)

# #find best based on HIV prev
# mse_df_top_hiv_prev<-mse_df2%>%
#   filter(hivprev_rank_total<=10)
# 
# outdir_best_analysis<-paste0(outdir_analysis, '/hiv_prev_best')
# outdir_best_analysis_ART<-paste0(outdir_best_analysis, '/ART_COVERAGE')
# outdir_best_analysis_HIV_PREV<-paste0(outdir_best_analysis, '/HIV_PREV')
# outdir_best_analysis_TB_MORT<-paste0(outdir_best_analysis, '/TB_MORT')
# 
...
# dir.create(file.path(outdir_best_analysis_TB_MORT))
# 
# top_graphs_func(mse_df_top_hiv_prev)
# 
# setwd(outdir_best_analysis)
# write.csv(mse_df_top_hiv_prev, paste0('mse_df_top_hiv_only_', start_eval_date,
#                                      '_.csv'), row.names = FALSE)
