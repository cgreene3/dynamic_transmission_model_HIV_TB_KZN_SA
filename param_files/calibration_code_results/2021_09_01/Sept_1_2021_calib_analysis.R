...
description <- '/iterative_art_init_calc_with_NO_hiv_prev_change'
...
indir_state_prog<-paste0("~/Documents/academic_posttt_2020/HIV_TB/calib_outdir/", start_eval_date, description)
...
mse_df3<-mse_df2%>%
  filter(hiv_prev_mse_male <= ((max(hiv_prev_mse_male)-min(hiv_prev_mse_male))*.5),
         hiv_prev_mse_female <= ((max(hiv_prev_mse_female)-min(hiv_prev_mse_female))*.5))
...
mse_df_top_tb_mort_hiv_neg_male<-mse_df2%>%
  filter(tbmort_HIV_neg_male<=10)
...
mse_df_top_tb_mort_hiv_neg_female<-mse_df2%>%
  filter(tbmort_HIV_neg_female<=10)
...
mse_df_top_tb_mort_hiv_pos_male<-mse_df2%>%
  filter(tbmort_HIV_pos_male<=10)
...
mse_df_top_tb_mort_hiv_pos_female<-mse_df2%>%
  filter(tbmort_HIV_pos_female<=10)
