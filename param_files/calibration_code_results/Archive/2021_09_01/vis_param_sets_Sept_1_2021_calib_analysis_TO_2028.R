#Calibration Analysis Sept 1 to 2028
...
start_eval_date <- '2021_09_01' #may need to change manually 
desc_in_dir<-'/iterative_art_init_calc_with_NO_hiv_prev_change/past_2018'
#if analysis date is later that start date (running calib prog)
desc_outdir <- '/iterative_art_init_calc_with_NO_hiv_prev_change/1990_2028_ANALYSIS'
...
indir_state_prog<-paste0("~/Documents/academic_posttt_2020/HIV_TB/calib_outdir/", start_eval_date, desc_in_dir)

outdir_analysis <- paste0(here(),'/param_files/calibration_code_results/', start_eval_date, '/analysis/', desc_outdir)
...
vis_all_top_graphs_func<-function(mse_df_top){
  ...
  mort_calibration_df_vis<-mort_calibration_df%>%
    ...
    summarise(max_rate = max(mort_rate_per_100K),
              ...
              sd_mort_rate = sd(mort_rate_per_100K),
              expected_rate = mean(mort_rate_per_100K))
  
  for (g in G_compartment){
    
    gender_temp<-if_else(g == 1, 'Males', 'Females')
    df_temp <- mort_calibration_df_vis%>%filter(G_compartment == g, year < 2028)
    
    df1<- df_temp%>%filter(HIV_status == 'neg')
    df2<- df_temp%>%filter(HIV_status == 'pos')
    ...
    graph_temp <- ggplot()+
      ...
      scale_color_manual(values=c('purple', 'green', 'darkorchid4', 'darkgreen'))+
      labs(title = paste0('Deaths, rate per 100K, ', gender_temp))+
      scale_x_continuous(name = 'time', breaks=seq(from = 1990, to = 2027, by = 5))+
      scale_y_continuous(name = 'mortality rate', limits = c(0, 600), breaks=(seq(0, 600, 100)))+
  ...
  #for (calib_group in unique(mort_calibration_df$calibration_group)){
    
    #color_temp = 'purple'
    
    #if(grepl('pos', calib_group)){
    #  color_temp = 'green'
    #} 
    
    
    #mort_plot_list[[n]] <- ggplot(mort_calibration_df_all%>%
    #                               filter(calibration_group == calib_group,
    #                                      year < 2028), 
    #                             aes(x=year, y=mort_rate_per_100K, 
     #                                group = year)) + 
     # geom_boxplot(color=color_temp)+
     # ggtitle(calib_group)
