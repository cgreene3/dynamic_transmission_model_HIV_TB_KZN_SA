...
#betas to test
beta_1_test<-seq(from = 16, to = 18, by = .4)
beta_2_test<-seq(from = 16, to = 18, by = .4)
...
write.csv(sim_calibration_ref_df, 'sim_calibration_ref_df.csv')
...
for(n in 105:nrow(sim_calibration_ref_df)){
  last_year<-1989
  sim_id<-n
  print(sim_id)
  ...
  write.csv(out_df, file = paste0('out_df_sim_id_', sim_id, '.csv'))
