#Sept 2nd 2021
#Calc HIV ART initiations iteratively
#reduce hiv base testing
...
HIV_transitions_param_func<-function(yr, N_t_r_h_g){
  ...
  for (g in G_SET){
    ...
    if(yr >= 2004){
      ...
      #hiv_prev_change<-hiv_prev_next_yr-(hiv_prev_current/sum(N_t_r_h_g))
...
tb_hiv_prog_calibration_model <- function(time, N_t_r_h_g, parms){
  ...
  if (current_yr > last_year){
      ...
      #only have mortality rates up to 2018
      if(current_yr < 2018){
        #deaths
...
#  summarise(total = sum(prop_of_pop))
  

#test<-seq(from = 1, to = 4700, by = 250)
#test<-c(4621, 4622, 4645)
#time_track<-c()
