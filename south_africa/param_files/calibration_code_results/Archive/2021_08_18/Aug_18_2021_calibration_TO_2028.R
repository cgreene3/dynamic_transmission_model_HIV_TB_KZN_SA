...
outdir<-paste0("~/Documents/academic_posttt_2020/HIV_TB/calib_outdir/", start_eval_date, desc)
...
birth_rate_df<-read.csv('birth_rate_df_to_2028.csv')

#these data frames change depending on combin of calib parameters
setwd(indir_params)
param_df <- read_excel("Epi_model_parameters.xlsx", sheet = 'model_matched_parameters')
hiv_transition_df<-read.csv('hiv_transmission_df_v4.csv')
...
HIV_transitions_param_func<-function(yr){
  ...
  for (g in G_SET){
    ...
    eta_i_h_g[1,2,g]<-as.integer(temp2$hiv_incidence*1000)/1000#temp2$hiv_incidence
    eta_i_h_g[2,4,g]<-temp2$eta_24
    eta_i_h_g[3,4,g]<-temp2$eta_34
    print(eta_i_h_g[1,2,g])
  }
  return(eta_i_h_g)
}
