...
#based on calibration results from December 7 2021 for calibration evaluation results 
#and code see param_files/calibration_code_results/2021_12_07/analysis 
...
hiv_transition_df<-read_excel('hiv_transmission_df.xlsx', sheet = 'incidence_rates')
...
best_calib_results_df<-read.csv('mse_df_top.csv')

#policy related data
policies_IPT_initiation_df<-read_excel("IPT_inititation_df.xlsx", sheet = 'policies')
policies_ART_initiation_df<-read_excel("hiv_transmission_df.xlsx", sheet = 'policies')
...
#3 Policy set description#
#1: Standard of care (IPT & ART)
#2: Community ART delivery, Standard of care (IPT)
#3: Community ART and IPT delivery

POLICY_SET<-1:3

#Tracks policy eval to change IPT and ART initiation rates appropriately in global env.
current_policy_eval<-1 #temp holder
...
IPT_initiation_function<-function(yr){
  ...
  for (g in G_SET){
    ...
    if (yr <= 2017){
      ...
      IPT_init_temp <- policies_IPT_initiation_df%>%
        filter(G_SET == g,
...
HIV_transitions_param_func<-function(yr, N_t_r_h_g){
  ...
  for (g in G_SET){
    ...
    ###get incidence rates###
    if (yr <= 2017){ #before 2017 incidence rates are the same (impacted by policy)
      #get hiv incidence
      ...
      #since no projections in 2029 keep at 2028 incidence estimates
      #so the code will run (will not effect results since 2029 is past eval)
      temp2<-hiv_transition_df%>%
        filter(gender == genders[g],
               year == 2028,
               policy_id == current_policy_eval)
    ...
    #get HIV progression CD4 counts (doesn't change over years)####
    temp <- param_df%>%
      filter(notation == 'eta',
             HIV_compartment == 23,
             P_compartment == 1,
    ...
    if(yr >= 2004){
      ...
      if(yr < 2016){
        ...
        n2_prop_eligible_current_yr <-1
      }
      
      #if before policy change
      if (yr < 2017){
        temp_hiv_transition_df<-hiv_transition_df%>%filter(policy_id == 1)
        n4_prop_next_yr <- as.double(temp_hiv_transition_df$art_coverage[nxt_yr_row_temp])
      }
      else if (yr == 2017){ ###still the same in the current year
        art_coverage_temp<-policies_ART_initiation_df%>%
          filter(POLICY_ID == 1,
                 G_SET == g)
        n4_prop_next_yr <-art_coverage_temp$art_coverage[1]
      }
      else {
        art_coverage_temp<-policies_ART_initiation_df%>%
          filter(POLICY_ID == current_policy_eval,
                 G_SET == g)
        n4_prop_next_yr <-art_coverage_temp$art_coverage[1]
      }
...
for(n in best_calib_results_df$sim_id){
  last_year<-1989
  ...
  for(p in POLICY_SET){
    ...
    out_df<- cbind(year = as.integer(start_yr+out_df$time), 
                   sim_id = rep(sim_id_temp2, times = nrow(out_df)),
    ...
    if(p == 1){
      ...
      out_df_all_policies<-rbind(out_df_all_policies, out_df)
    }
  }
  write.csv(out_df_all_policies, file = paste0('out_df_sim_id_', sim_id_temp2, '.csv'), row.names = FALSE)
}
