...
art_initiation_related_param_df <-
  read.csv('art_initiation_related_param_df.csv')
...
HIV_transitions_param_func<-function(yr, N_t_r_h_g){
  ...
  for (g in G_SET){
    ...
    eta_i_h_g[1,2,g]<-hiv_incidence_temp$hiv_incidence
    } else if (yr == 2029){
    ...
    eta_i_h_g[2,3,g]<-hiv_prog_temp$Reference_expected_value
    ...
    if(yr >= 2004){
      ...
      if(yr <= 2017){
        art_coverage_temp<-art_initiation_related_param_df%>%
      ...
      #before 2016 not every PLWH eligible
      perc_hiv_compartment_2_eligible_temp<-1
      if(yr < 2016){
        perc_hiv_compartment_2_eligible_temp<-art_initiation_related_param_df%>%
          filter(G_compartment == g,
                 year == yr,
                 POLICY_ID == 1)
        perc_hiv_compartment_2_eligible_temp<-
          perc_hiv_compartment_2_eligible_temp$percent_hiv_compartment_2_eligible
      } 
      
      
      art_initiation_rate_all_denominator<-(perc_hiv_compartment_2_eligible_temp*
                                              n2_prop_current_yr)+
        (n3_prop_current_yr)
      ...
      eta_i_h_g[2,4,g]<-art_initiation_rate_all*perc_hiv_compartment_2_eligible_temp
...
#will be updated with each calib simulation to account for multipliers
