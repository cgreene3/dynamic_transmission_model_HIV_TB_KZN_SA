...
out_df<- cbind(year = as.integer(start_yr+out_df$time), out_df)
...
summarised_metrics_TB<-out_df%>%
  ...
  mutate(Tb_inc_neg_female_Y = Tb_inc_neg_female_max-lag(Tb_inc_neg_female_max),
            Tb_inc_neg_male_Y = Tb_inc_neg_male_max-lag(Tb_inc_neg_male_max),
            Tb_inc_pos_female_Y = Tb_inc_pos_female_max-lag(Tb_inc_pos_female_max),
            Tb_inc_pos_male_Y = Tb_inc_pos_male_max-lag(Tb_inc_pos_male_max),
            Tb_mort_neg_female_Y = Tb_mort_neg_female_max-lag(Tb_mort_neg_female_max),
            Tb_mort_neg_male_Y = Tb_mort_neg_male_max-lag(Tb_mort_neg_male_max),
            Tb_mort_pos_female_Y = Tb_mort_pos_female_max-lag(Tb_mort_pos_female_max),
            Tb_mort_pos_male_Y = Tb_mort_pos_male_max-lag(Tb_mort_pos_male_max))%>%
  filter(year == 2006 | year == 2017)%>%
  select(c('year', 'Tb_inc_neg_female_Y',
           'Tb_inc_neg_male_Y',
            'Tb_inc_pos_female_Y',
            'Tb_inc_pos_male_Y',
            'Tb_mort_neg_female_Y',
            'Tb_mort_neg_male_Y', 
            'Tb_mort_pos_female_Y', 
            'Tb_mort_pos_male_Y'))

summarised_metrics_hiv_prev<-out_df%>%
  mutate(time_temp = time%%1)%>%
  filter(time_temp == .5)%>%
  mutate(hiv_prev_female = hiv_prev_female/total_female_pop,
         hiv_prev_male = hiv_prev_male/total_male_pop)%>%
  filter(year == 2006 | year == 2017)%>%
  select(c('year', 'hiv_prev_female', 'hiv_prev_male'))
...
write.csv(summarised_calib_metrics_df, file = paste0('summarised_calib_metrics_df_sim_id', 
                                                sim_id_current_eval, '.csv'), row.names = FALSE)
