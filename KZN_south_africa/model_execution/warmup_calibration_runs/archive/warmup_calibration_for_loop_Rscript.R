...
#location where want outputs
outdir<-paste0(here(), '/calibration_outputs_test/',calib_run_name[calib_run_id])
...
sim_calibration_ref_df<-read.csv(paste0('sim_calibration_ref_df_', calib_run_name[calib_run_id] ,'.csv'))
...
if(calib_run_id == 1){
  ...
  param_df<- read.csv('KZN_SA_model_parameters_round_2.csv')
}
...
beta_g_param_extraction<-function(sim_id_current_eval){
  ...
  for (g in G_SET){
    ...
    print(g)
    print(temp_param_df$max[1])
    print(temp_param_df$min[1])
    print(temp_sim_df[1])
  }
  return(beta_g)
}
...
tb_hiv_prog_calibration_model <- function(time, N_t_r_h_g, parms){
  ...
  dN_t_r_h_g[Tb_inc_neg_male_loc]<-dN_t_r_h_g[129]<- (theta_h[1]*pi_i_t[3,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[3,1:2,1,1]])) + #from recent TB infection to active 
    ...
    (pi_i_t[5,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[5,1:2,1,1]])) + #from TB infection on IPT to active
    (pi_i_t[8,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[8,1:2,1,1]])) #from TB after on IPT to active
  ...
  dN_t_r_h_g[Tb_inc_neg_female_loc]<- (theta_h[1]*pi_i_t[3,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[3,1:2,1,2]])) + #from recent TB infection to active 
    ...
    (pi_i_t[5,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[5,1:2,1,2]])) + #from TB infection on IPT to active
    (pi_i_t[8,6]*sum(N_t_r_h_g[N_t_r_h_g_ref[8,1:2,1,2]])) #from TB after on IPT to active
...
for(sim_id_current_eval in sim_id_start:sim_id_end){
  ...
  setwd(outdir)
  write.csv(summarised_calib_metrics_df, file = paste0('calib_metrics_df_', calib_run_name[calib_run_id], '_simid_', sim_id_current_eval, '.csv'), row.names = FALSE)
}
