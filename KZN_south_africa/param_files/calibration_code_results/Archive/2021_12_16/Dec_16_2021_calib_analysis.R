...
ggplot(mse_df3%>%arrange(hiv_prev_mse_male_norm_rank), 
       aes(x=hiv_prev_mse_male_norm_rank, y=hiv_prev_mse_male_norm)) +
  geom_line()

ggplot(mse_df3%>%arrange(hiv_prev_mse_female_norm_rank), 
...
write.csv(mse_df3, 'mse_df_filtered.csv')
...
test_lower_contacts<-sim_calibration_ref_df%>%
  filter(b1_contact == 6,
         b2_contact == 6)

top_df_all_info_df<-data.frame()

lapply(test_lower_contacts$sim_calib_id, function(sim_id_itr){  #unique(mse_df_top$sim_id)
  file_name<-paste0('out_df_sim_id_', sim_id_itr, '.csv')
  print(sim_id_itr)
  temp_data <- fread(file_name) 
  temp_data$sim_id <- rep(sim_id_itr, times = nrow(temp_data))
