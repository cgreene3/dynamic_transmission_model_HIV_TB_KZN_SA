...
tb_hiv_prog_calibration_model <- function(time, N_t_r_h_g, parms){
  ...
  if (current_yr > last_year){
      ...
      #only have mortality rates and IPT initiation rates up to 2018
      if(current_yr < 2018){
        #deaths
...
for(n in sim_calibration_ref_df$sim_calib_id){
  ...
  write.csv(out_df, file = paste0('out_df_sim_id_', sim_id_temp2, '.csv'), row.names = FALSE)
