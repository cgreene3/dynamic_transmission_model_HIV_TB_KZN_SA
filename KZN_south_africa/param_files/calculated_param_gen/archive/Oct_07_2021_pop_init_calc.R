...
current_date <- '2021_10_07'
outdir <- paste0(here(),'/param_files/calibration_code_results/', current_date)
...
#read in data (read in data is store in the same place as pop init df)
...
pop_df<-read.csv('pop_df.csv') #for init gender adjustments
...
mort_df$G_compartment<-as.integer(mort_df$sex_id)
...
alpha_in_t_r_h_g <- array(data = 0, c(length(TB_SET), 
                                      ...
                                      length(HIV_SET), 
                                      length(G_SET)))
reset_birth_rates_fun<-function(){
  alpha_in_t_r_h_g <- array(data = 0, c(length(TB_SET), 
                                        length(DR_SET), 
                                        length(HIV_SET), 
                                        length(G_SET)))
...
for(n in sim_calibration_ref_df$sim_calib_id){
    ...
    #set initial birth rates
    alpha_in_t_r_h_g<<-reset_birth_rates_fun()
