...
birth_rate_hiv_indir<-paste0(here(),'/param_files/birth_rate_gen')
...
setwd(birth_rate_hiv_indir)
#hiv adjustments overtime
hiv_prop_df<-read_excel('HIV_prev_birth_rate.xlsx')
...
birth_rate_df<-do.call('rbind', 
                       replicate(2018-1990+1, 
                                 birth_rate_df, simplify = FALSE))

birth_rate_df$year <- rep(1990:2018, each = 128)

hiv_adj<-c()

for (row in 1:nrow(birth_rate_df)){
  yr_temp<- birth_rate_df[row, 'year']
  g_temp <- birth_rate_df[row, 'G_compartment']
  if (birth_rate_df[row, 'HIV_compartment'] == 1){
    hiv_adj <- c(hiv_adj, (1-unique(hiv_prop_df$hiv_prevalence[(hiv_prop_df$year == yr_temp) &
                                                      (hiv_prop_df$G_compartment== g_temp)])))
