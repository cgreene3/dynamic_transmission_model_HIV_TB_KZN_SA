#Policy Analysis Oct 15 to 2028
      ...
      for (p in c(1,2,3)){
        ...
        graph_temp<-graph_temp+
          geom_ribbon(data=df_temp,aes(x = year, ymin = min_rate, ymax = max_rate), 
                      inherit.aes = FALSE, fill = colors_for_graph[p])
      }
      ...
      df_temp2<-TB_mort_df_vis%>%
        ...
        graph_temp<-graph_temp+
          geom_line(data = df_temp, aes(x = year, y = expected_rate, 
                                        color = policy_name))+
          scale_color_manual(values=c(colors_for_graph[c(2,4,6)]))
      
      max_graph = if_else(hs == 'negative', 150, 400)
      breaks_graph=if_else(hs == 'negative', 50, 100)
