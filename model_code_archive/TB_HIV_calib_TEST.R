...
outdir<-paste0(here(),'/model_outputs/calibration/', start_eval_date, '/state_prog_outputs')
...
for(n in 5893){#1:nrow(sim_calibration_ref_df)){
  last_year<-1989
  print(n)
...
state_prog_df <-reshape2::melt(out_df, 
                               id.vars = c("time"))
...
state_prog_df$year<-as.integer(state_prog_df$time+1990)
...
TB_incidence_df%>%
  ggplot(aes(x = year, y = incidence_rate_per_100K,
