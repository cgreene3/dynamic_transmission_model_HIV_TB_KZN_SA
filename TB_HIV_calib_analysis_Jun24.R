...
start_eval_date <- '07_11_2021'
description <-'IncludeARTMort18'#constant_births_try1' #'n2birthrates'#'female_hiv_prog_8Point5'
...
indir_state_prog<-"~/Documents/academic_posttt_2020/HIV_TB/calib_outdir/07_11_2021"
...
sim_calibration_ref_df$mtbh4_calib_id<-if_else(sim_calibration_ref_df$mtbh4_calib_id == 25, as.integer(17), sim_calibration_ref_df$mtbh4_calib_id)
...
#test<-c(4621, 4622, 4645)#seq(from = 1, to = 4700, by = 250)
#######mse df function#####
...
#####Evaluating Top MSE#####
#setwd(paste0(here(),'/param_files/calib_ref_data/', start_eval_date, '/analysis'))
#mse_df<-read_xlsx('mse_df.xlsx')

mse_df_top<-mse_df2%>%
  filter(tbmort_rank_total<=10)

outdir_best_analysis<-paste0(outdir_analysis, '/tb_best')
...
#mse_df_top<-mse_df_top%>%
 # left_join(sim_calibration_ref_df%>%
  #            rename(sim_id = sim_calib_id), 
   #         by = c('sim_id'))

write.csv(mse_df_top, 'mse_df_top1.csv', row.names = FALSE)
